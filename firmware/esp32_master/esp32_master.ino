...
const char* BACKEND_BASE = "https://fpoly-smartfarm.id.vn";
...
// ================================================================
// BIẾN VÀ HÀM HỖ TRỢ OLED 4 DÒNG
// ================================================================
unsigned long lastAutoOLEDRefresh = 0;
const unsigned long AUTO_OLED_REFRESH_INTERVAL = 1000UL; 
...
// KHÔI PHỤC LẠI HÀM LORA QUEUE GỐC (Tránh lỗi kẹt buffer thủ công)
void processLoRaQueue() {
  ...
  if (commandToSend.length() > 0) {
    ...
    // OLED Delay cực kỳ quan trọng cho mạch UART đẩy data LoRa E32
    if(currentCommand != "<A:GET_DATA>") updateOLED("LORA TX:", currentCommand);
  }
}

// ================================================================
// THUẬT TOÁN TRỘN PHÂN
// ================================================================
void handleAutoMixingLogic() {
  if (systemMode != 1) return; // Đã loại bỏ Timeout 5s khiến hệ thống đóng băng
  unsigned long now = millis();
  const float WATER_FULL_DIST = 20.0; 

  switch (mixState) {
    case MIX_IDLE: 
      if (!isMixingReady) {
        mixState = MIX_START_PUMP3; 
        showAutoStep("TRON PHAN: B1", "Kiem tra muc nuoc...", "", "");
      }
      break;
    ...
    case MIX_PUMPING_WATER: {
      ...
      if ((Dist4 > 0 && Dist4 <= WATER_FULL_DIST) || elapsed >= 1200000UL) { 
        mixState = MIX_STOP_PUMP3; 
      } else if (autoOLEDShouldTick()) {
        updateOLED4("TRON PHAN: B1", "Dang cap nuoc...", "Muc: " + String(Dist4, 1) + "cm", "Cho day bon (<20cm)");
      }
      break;
    }
    ...
    case MIX_WAIT_STABLE: {
      ...
      if (elapsed >= 20000UL) {
        ...
        if (EC_Value < ecMin) { 
          ...
          mixState = MIX_START_PUMP3; 
          showAutoStep("TRON PHAN", "EC cao -> Pha loang", "Cap them nuoc...", "");
        } 
        else { 
          isMixingReady = true; 
          mixState = MIX_IDLE; 
        ...
        unsigned long remainSec = (20000UL - elapsed) / 1000UL;
        updateOLED4("TRON PHAN: B2", "Khuay on dinh nuoc", "EC: " + String(EC_Value, 2), "Con lai: " + String(remainSec) + "s");
      }
      break;
    }
  ...
  switch (autoState) {
    case AUTO_IDLE: 
      if (Humidity <= humMin && isMixingReady) { 
        if (Dist4 > 0 && Dist4 >= WATER_EMPTY_DIST) {
          if (autoOLEDShouldTick()) updateOLED4("TUOI CAY: LOI", "Can nuoc trong bon!", "Vui long cap nuoc...", "");
          isMixingReady = false; 
          return;
        }
        currentValveIdx = 0; 
...
void pollWebCommands() {
  ...
  for (JsonObject cmd : doc.as<JsonArray>()) {
    ...
    if (devId == "settings" || action.startsWith("SAVE=") || action.startsWith("{")) {
...
// KHÔI PHỤC BẮT LỖI ESTOP LORA GỐC
  ...
  while (loraSerial.available()) {
    ...
    if (c == '\n' || c == '\r') {
      ...
      if (espLoraRxBuffer.length() > 0) {
        ...
        // Bắt lệnh ESTOP chủ động từ STM32
        if (espLoraRxBuffer == "<B:ESTOP>") {
            enqueueEmergencyStop();
            updateOLED4("BAO DONG LORA", "STM32 bao Mua To!", "", "Da ngat he thong!");
        }
...
void processNextionCommand(String rawCmd) {
  ...
  if (cmdUpper.indexOf("CMD=RESTORE") >= 0) {
    ...
    if (currentPage == 2) updateNextionSettingsPage(); 
    enqueueCommand("<SET_DATA=5.5,6.5,1.5,3.0,22.0,35.0,100.0,100.0,1,0>"); 
    settingsPending = true; 
    return;
  }
}
...
void loop() {
  ...
  // ==========================================================
  // MÔ PHỎNG MỰC NƯỚC: THỰC TẾ 1 PHÚT ĐỘC LẬP
  // ==========================================================
  ...
  if (millis() - lastSimTime >= 1000) { 
    ...
    if (mixState == MIX_PUMPING_WATER) { 
        simDist4 -= 1.0;  
        if (simDist4 < 10.0) simDist4 = 10.0; 
    }
    // Bơm 4 chạy (tưới) -> Khoảng cách tăng 1.0cm/s (nước rút đi)
    else if (autoState == AUTO_IRRIGATING) {
        simDist4 += 1.0; 
        if (simDist4 > 85.0) simDist4 = 85.0; 
    }
    
    // Ghi đè tuyệt đối giá trị mô phỏng
    Dist4 = simDist4; 
    xSemaphoreGive(systemMutex);
