...
const char* BACKEND_BASE = "http://192.168.137.11:4000";
...
enum AutoState { AUTO_IDLE, AUTO_OPEN_VALVE, AUTO_WAIT_VALVE_ON, AUTO_WAIT_PUMP_ON, AUTO_IRRIGATING, AUTO_WAIT_PUMP_OFF, AUTO_WAIT_VALVE_OFF, AUTO_WAIT_RESTING_CMD, AUTO_RESTING };
AutoState autoState = AUTO_IDLE; 
unsigned long autoStateTimer = 0;

enum MixState { MIX_IDLE, MIX_START_PUMP3, MIX_PUMPING_WATER, MIX_STOP_PUMP3, MIX_WAIT_STABLE, MIX_START_DOSING, MIX_DOSING_NUTRIENT, MIX_STOP_DOSING };
...
void sendText(const String& obj, const String& txt) { 
  Serial.println("[TX] " + obj + " = " + txt);
  nextion.print(obj + "=\"" + txt + "\""); 
...
// ================================================================
// OLED 4 DONG (128x32, dung het man hinh) - danh cho hien thi chi tiet
// tung buoc cua che do AUTO (mo phong tren du lieu that tu STM32).
// ================================================================
void updateOLED4(const String& line1, const String& line2, const String& line3, const String& line4) {
  display.clearDisplay(); display.setTextSize(1); display.setTextColor(SSD1306_WHITE);
  display.setCursor(0, 0);  display.println(line1);
...
unsigned long lastAutoOLEDRefresh = 0;
const unsigned long AUTO_OLED_REFRESH_INTERVAL = 1000UL; // Cap nhat OLED 1 lan/giay khi dang cho

// Hien thi 1 buoc AUTO va reset dong ho lam moi (dung khi CHUYEN trang thai)
void showAutoStep(const String& l1, const String& l2, const String& l3, const String& l4) {
  updateOLED4(l1, l2, l3, l4);
  lastAutoOLEDRefresh = millis();
}

// Chi cho phep lam moi OLED moi 1 giay 1 lan (dung trong cac trang thai CHO/DEM NGUOC)
bool autoOLEDShouldTick() {
  if (millis() - lastAutoOLEDRefresh >= AUTO_OLED_REFRESH_INTERVAL) {
    lastAutoOLEDRefresh = millis();
    return true;
  }
  return false;
...
void updateDashboard() {
  ...
  sendText("t2.txt", String(pH_Value, 1)); sendText("t3.txt", String(EC_Value, 2));
...
void safelySwitchMode(int newMode) {
  ...
  if (newMode == 0) { 
    updateOLED("CHUYEN CHE DO", "-> MANUAL"); 
    if (currentPage == 3) updateNextionControlStates(); 
    loraSerial.println(EMERGENCY_COMMAND); delay(50); 
  } 
  else { 
    updateOLED("CHE DO AUTO", "Dang hoat dong..."); 
    if (currentPage == 3) updateNextionControlStates(); 
    lastLoraRxTime = 0; 
...
void enqueueEmergencyStop() {
  ...
  // Bắn lệnh trực tiếp cho Arduino Nano đóng toàn bộ Relay
  ...
  // [ĐÃ SỬA]: Kích hoạt cờ này để ESP32 đẩy trạng thái OFF của tất cả bơm/van lên Web ngay lập tức
...
// ================================================================
// THUẬT TOÁN TRỘN PHÂN SIÊU MƯỢT (CHỐNG NGẬP LỤT LỆNH QUEUE)
// Sử dụng cơ chế kích hoạt 1 lần (Trigger-once) qua từng State
// ================================================================
void handleAutoMixingLogic() {
  ...
  switch (mixState) {
    case MIX_IDLE: 
      // [BỔ SUNG BƯỚC 3]: Kiểm tra Bồn Nước (Dist3) có nước mới cho bắt đầu pha
      if (!isMixingReady && Dist3 > 0 && Dist3 < WATER_EMPTY_DIST) { 
        mixState = MIX_START_PUMP3; 
        statusReportPending = true; // Báo web cập nhật thanh tiến trình
      } 
      break;
    ...
    case MIX_START_PUMP3:
      ...
      showAutoStep("AUTO TRON B1/4", "Bom nuoc vao binh", "EC:" + String(EC_Value, 2) + " Muc:" + String(ecMin, 1) + "-" + String(ecMax, 1), "Thoi gian: 5s");
    ...
    case MIX_PUMPING_WATER: {
      ...
      if (elapsed >= 5000UL) { 
        mixState = MIX_STOP_PUMP3; 
        statusReportPending = true;
      } else if (autoOLEDShouldTick()) {
        int remain = (5000UL - elapsed) / 1000UL + 1;
        updateOLED4("AUTO TRON B1/4", "Dang bom nuoc...", "EC:" + String(EC_Value, 2) + " Muc:" + String(ecMin, 1) + "-" + String(ecMax, 1), "Con lai: " + String(remain) + "s");
      }
      break;
    }

    case MIX_STOP_PUMP3:
      enqueueCommand("<B:OFF3>"); 
      mixStateTimer = now; 
      mixState = MIX_WAIT_STABLE; 
      showAutoStep("AUTO TRON B2/4", "Tat bom, cho on dinh", "EC:" + String(EC_Value, 2) + " Muc:" + String(ecMin, 1) + "-" + String(ecMax, 1), "Thoi gian: 5s");
      statusReportPending = true;
      break;
    ...
    case MIX_WAIT_STABLE: {
      ...
      if (elapsed >= 5000UL) {
        if (EC_Value < ecMin) { 
          ...
          mixState = MIX_START_PUMP3; 
          showAutoStep("AUTO TRON B2/4", "EC qua cao -> pha loang", "EC:" + String(EC_Value, 2) + " > Max:" + String(ecMax, 1), "Lap lai buoc 1...");
        } 
        else { 
          isMixingReady = true; 
          mixState = MIX_IDLE; 
          showAutoStep("AUTO TRON: XONG!", "EC dat chuan", "EC:" + String(EC_Value, 2) + " Muc:" + String(ecMin, 1) + "-" + String(ecMax, 1), "San sang tuoi...");
        }
        statusReportPending = true;
      } else if (autoOLEDShouldTick()) {
        int remain = (5000UL - elapsed) / 1000UL + 1;
        updateOLED4("AUTO TRON B2/4", "Cho nuoc on dinh", "EC:" + String(EC_Value, 2) + " Muc:" + String(ecMin, 1) + "-" + String(ecMax, 1), "Con lai: " + String(remain) + "s");
      }
      break;
    }

    case MIX_START_DOSING:
      enqueueCommand("<B:ON1>"); 
      enqueueCommand("<B:ON2>"); 
      enqueueCommand("<B:ON5>"); 
      mixStateTimer = now; 
      mixState = MIX_DOSING_NUTRIENT; 
      showAutoStep("AUTO TRON B3/4", "Cham phan + khuay", "EC:" + String(EC_Value, 2) + " -> Muc:" + String(ecMin, 1), "Thoi gian: 3s");
      statusReportPending = true;
      break;
    ...
    case MIX_DOSING_NUTRIENT: {
      ...
      if (elapsed >= 3000UL) { 
        ...
        int remain = (3000UL - elapsed) / 1000UL + 1;
        updateOLED4("AUTO TRON B3/4", "Dang cham phan...", "EC:" + String(EC_Value, 2) + " -> Muc:" + String(ecMin, 1), "Con lai: " + String(remain) + "s");
      }
      break;
    }

    case MIX_STOP_DOSING:
      enqueueCommand("<B:OFF1>"); 
      enqueueCommand("<B:OFF2>"); 
      enqueueCommand("<B:OFF5>"); 
      mixStateTimer = now; 
      mixState = MIX_WAIT_STABLE; 
      showAutoStep("AUTO TRON B4/4", "Tat phan, do lai EC", "EC hien tai:" + String(EC_Value, 2), "Cho 5s...");
      statusReportPending = true;
      break;
  }
}
// ================================================================
// THUẬT TOÁN TƯỚI SIÊU MƯỢT (CHỐNG NGẬP LỤT LỆNH QUEUE)
// ================================================================
void handleAutoIrrigationLogic() {
  if (systemMode != 1 || millis() - lastLoraRxTime > 5000UL) return; 

  // [BỔ SUNG BƯỚC 1]: KHÓA CHÉO AN TOÀN (Cạn nước Bồn Trộn hoặc Mưa to)
  bool isTankEmpty = (Dist4 > WATER_EMPTY_DIST && Dist4 > 0); 
  bool isHeavyRain = (RainPercent >= RAIN_MAX_PERCENT);

  if (isTankEmpty || isHeavyRain) {
    if (autoState != AUTO_IDLE) {
      enqueueCommand("<B:OFF4>"); // Tắt Bơm Tưới
      enqueueCommand("<B:OFF6>"); // Tắt Van Tưới
      autoState = AUTO_IDLE;
      showAutoStep("KHOA AN TOAN", isTankEmpty ? "Bon tron can nuoc!" : "Troi dang mua to!", "Da ngat bom va van", "Cho on dinh...");
      statusReportPending = true;
    }
    return; // Cắt ngang, cấm tưới!
  }

  unsigned long now = millis();
  switch (autoState) {
    case AUTO_IDLE: 
      if (Humidity <= humMin && isMixingReady) { 
    ...
    case AUTO_OPEN_VALVE:
      enqueueCommand("<B:ON6>"); 
      autoStateTimer = now; 
      autoState = AUTO_WAIT_VALVE_ON; 
      showAutoStep("AUTO TUOI B1/4", "Dang mo van tuoi", "Do am:" + String(Humidity, 1) + "% Muc:<=" + String(humMin, 1) + "%", "Cho van mo: 3s");
      statusReportPending = true;
      break;
    ...
    case AUTO_WAIT_VALVE_ON: {
      ...
      if (elapsed >= 3000UL) { 
        autoState = AUTO_WAIT_PUMP_ON; 
        statusReportPending = true;
      } else if (autoOLEDShouldTick()) {
        int remain = (3000UL - elapsed) / 1000UL + 1;
        updateOLED4("AUTO TUOI B1/4", "Dang mo van tuoi", "Do am:" + String(Humidity, 1) + "%", "Con lai: " + String(remain) + "s");
      }
      break;
    }

    case AUTO_WAIT_PUMP_ON:
      enqueueCommand("<B:ON4>"); 
      autoStateTimer = now; 
      autoState = AUTO_IRRIGATING; 
      showAutoStep("AUTO TUOI B2/4", "Bat bom, bat dau tuoi", "Do am:" + String(Humidity, 1) + "% Muc:>=" + String(humMax, 1) + "%", "Toi da: " + String(timeBom) + " phut");
      statusReportPending = true;
      break;

    case AUTO_IRRIGATING: {
      unsigned long maxMs = (unsigned long)timeBom * 60000UL;
      unsigned long elapsed = now - autoStateTimer;
      if (Humidity >= humMax || elapsed >= maxMs) { 
        autoState = AUTO_WAIT_PUMP_OFF; 
        statusReportPending = true;
      } else if (autoOLEDShouldTick()) {
        unsigned long remainSec = (maxMs > elapsed) ? (maxMs - elapsed) / 1000UL : 0;
        updateOLED4("AUTO TUOI B3/4", "Dang tuoi nuoc...", "Do am:" + String(Humidity, 1) + "% / " + String(humMax, 1) + "%", "Con lai: " + String(remainSec) + "s (toi da)");
      }
      break;
    }
    ...
    case AUTO_WAIT_PUMP_OFF:
      ...
      showAutoStep("AUTO TUOI B3/4", "Da du nuoc, tat bom", "Do am:" + String(Humidity, 1) + "%", "Cho dong van: 3s");
      statusReportPending = true;
      break;
    ...
    case AUTO_WAIT_VALVE_OFF: {
      ...
      if (elapsed >= 3000UL) { 
        ...
        int remain = (3000UL - elapsed) / 1000UL + 1;
        updateOLED4("AUTO TUOI B3/4", "Dang tat bom...", "Do am:" + String(Humidity, 1) + "%", "Con lai: " + String(remain) + "s");
      }
      break;
    }

    case AUTO_WAIT_RESTING_CMD:
      enqueueCommand("<B:OFF6>"); 
      autoStateTimer = now; 
      if (Humidity < humMax) { 
          autoState = AUTO_RESTING; 
          showAutoStep("AUTO TUOI B4/4", "Da dong van, nghi ngoi", "Do am:" + String(Humidity, 1) + "%", "Nghi: " + String(timeNghi) + " phut"); 
      } 
      else { 
          autoState = AUTO_IDLE; 
          isMixingReady = false; 
          showAutoStep("AUTO TUOI: XONG!", "Da dong van", "Do am dat chuan: " + String(Humidity, 1) + "%", "Cho lan tuoi ke tiep"); 
      } 
      statusReportPending = true;
      break;

    case AUTO_RESTING: {
      unsigned long maxMs = (unsigned long)timeNghi * 60000UL;
      unsigned long elapsed = now - autoStateTimer;
      if (elapsed >= maxMs) { 
        autoState = AUTO_IDLE; 
        statusReportPending = true;
        ...
        unsigned long remainSec = (maxMs > elapsed) ? (maxMs - elapsed) / 1000UL : 0;
        updateOLED4("AUTO TUOI B4/4", "Dang nghi ngoi...", "Do am hien tai:" + String(Humidity, 1) + "%", "Con lai: " + String(remainSec) + "s");
      }
      break;
    }
...
void pollWebCommands() {
  ...
  for (JsonObject cmd : doc.as<JsonArray>()) {
    ...
    // THÊM 2 DÒNG NÀY ĐỂ FIX LỖI "WAS NOT DECLARED"
    ...
    // 2. DỪNG KHẨN CẤP (ESTOP) - [ĐÃ SỬA LỖI TREO WEB]
    ...
    if (action == "FETCH_NOW" || action == "GET_DATA" || devId == "data" || devId == "system") {
      if (!isCommandInQueue("<A:GET_DATA>")) {
        ...
        updateOLED("WEB CMD", "Lay du lieu...");
      }
      webCommandId = id;
      return;
    }

   if (devId == "settings" || action.startsWith("SAVE=") || action.indexOf("{") >= 0) {
      Serial.println("\n[WEB GỬI CÀI ĐẶT]: " + action);
      
      if (action.indexOf("{") >= 0) {
        DynamicJsonDocument conf(512);
        if (!deserializeJson(conf, action)) {
          phMin = conf["phMin"] | phMin; phMax = conf["phMax"] | phMax;
          ecMin = conf["ecMin"] | ecMin; ecMax = conf["ecMax"] | ecMax;
          tempMin = conf["tempMin"] | tempMin; tempMax = conf["tempMax"] | tempMax;
          humMin = conf["humMin"] | humMin; humMax = conf["humMax"] | humMax;
          timeBom = conf["timeBom"] | timeBom; timeNghi = conf["timeNghi"] | timeNghi;
        }
      } 
      else {
        String data = action;
        if (data.startsWith("SAVE=")) data = data.substring(5);
        phMin = getValue(data, ',', 0).toFloat(); phMax = getValue(data, ',', 1).toFloat();
        ecMin = getValue(data, ',', 2).toFloat(); ecMax = getValue(data, ',', 3).toFloat();
        tempMin = getValue(data, ',', 4).toFloat(); tempMax = getValue(data, ',', 5).toFloat();
        humMin = getValue(data, ',', 6).toFloat(); humMax = getValue(data, ',', 7).toFloat();
        timeBom = getValue(data, ',', 8).toInt(); timeNghi = getValue(data, ',', 9).toInt();
      }
    ...
    if (devId.startsWith("pump") || devId.startsWith("van")) {
      ...
      if (devId.startsWith("pump")) { relayPin = devId.substring(4).toInt(); } 
      else if (devId.startsWith("van")) { relayPin = devId.substring(3).toInt() + 5; }
...
void processLoRaData(const String& incomingData) {
  ...
  EC_Value = getValue(raw, ',', 2).toFloat(); 
  ...
  if (currentPage == 1) updateDashboard(); 
...
void updateNextionSettingsPage() {
  // Nếu đang đứng ở trang Settings (2), bỏ tiền tố page5. đi để Nextion không báo lỗi
...
void processNextionCommand(String rawCmd) {
  ...
  if (cmdUpper.indexOf("SETTINGS_READY") >= 0) { 
    ...
    // BÍ QUYẾT NẰM Ở ĐÂY:
    // Nếu trước đó đang ở trang khác (currentPage khác 2), thì mới gửi số cũ lên màn hình.
    // Nếu vừa gõ bàn phím xong (currentPage vẫn đang là 2), thì KHÔNG LÀM GÌ CẢ để giữ nguyên số vừa gõ!
    if (currentPage != 2) { 
  ...
  if (cmdUpper.startsWith("BOM")) {
    ...
    if (eqRaw >= 0 && eqRaw + 1 < rawCmd.length()) {
      ...
      } else if (currentPage == 3) updateNextionControlStates(); 
    } return;
  }
  ...
  int saveIdx = cmdUpper.indexOf("SAVE=");
  String dataToSave = "";
  
  if (saveIdx >= 0) {
    dataToSave = cleanCmd.substring(saveIdx + 5);
  } else if (cleanCmd.indexOf(",") > 0 && cleanCmd.length() > 10) {
    dataToSave = cleanCmd;
  }
  ...
  if (dataToSave != "") {
    ...
    float nPhMin = getValue(dataToSave, ',', 0).toFloat();
    float nPhMax = getValue(dataToSave, ',', 1).toFloat();
    float nEcMin = getValue(dataToSave, ',', 2).toFloat();
    float nEcMax = getValue(dataToSave, ',', 3).toFloat();
    float nTempMin = getValue(dataToSave, ',', 4).toFloat();
    float nTempMax = getValue(dataToSave, ',', 5).toFloat();
    float nHumMin = getValue(dataToSave, ',', 6).toFloat();
    float nHumMax = getValue(dataToSave, ',', 7).toFloat();
    int nTimeBom = getValue(dataToSave, ',', 8).toInt();
    int nTimeNghi = getValue(dataToSave, ',', 9).toInt();

    if (nTimeBom <= 0) nTimeBom = 1; 
    if (nTimeNghi < 0) nTimeNghi = 0;
    
    phMin = nPhMin; phMax = nPhMax; ecMin = nEcMin; ecMax = nEcMax; 
    tempMin = nTempMin; tempMax = nTempMax; humMin = nHumMin; humMax = nHumMax; 
    timeBom = nTimeBom; timeNghi = nTimeNghi;
  ...
  if (cmdUpper.indexOf("CMD=RESTORE") >= 0) {
    // Dong bo voi nguong DEMO trong setup() de trigger nhanh khi test
    phMin = 5.5; phMax = 6.5; 
    ecMin = 0.5; ecMax = 3.5;     
    tempMin = 22.0; tempMax = 35.0; 
    humMin = 90.0; humMax = 92.0; 
    timeBom = 1; timeNghi = 0;     
    ...
    if (currentPage == 2) updateNextionSettingsPage(); 
    enqueueCommand("<SET_DATA=5.5,6.5,0.5,3.5,22.0,35.0,90.0,92.0,1,0>"); 
    settingsPending = true; 
    return;
  }
}
...
void setup() {
  ...
  // NGUONG DEMO: cai dat de trigger NHANH (vai chuc giay) thay vi phai
  // cho du lieu cam bien that dat gia tri san xuat thuc te. Chinh lai
  // cho phu hop khi chay that (xem ghi chu ben duoi).
  phMin = 5.5; phMax = 6.5;              // (chua duoc dung trong logic AUTO hien tai)
  ecMin = 0.5; ecMax = 3.5;              // bang rong -> buoc TRON EC ket thuc ngay sau 5-10s
  tempMin = 22.0; tempMax = 35.0;        // (chua duoc dung trong logic AUTO hien tai)
  humMin = 90.0; humMax = 92.0;          // bang hep, gan 100% -> tuoi kich hoat gan nhu ngay lap tuc
  timeBom = 1; timeNghi = 0;             // toi da 1 phut moi lan tuoi, khong nghi -> lap lai lien tuc
...
void loop() {
  ...
  if (systemMode == 1) { 
    // SUA LOI: truoc day dung AUTO_POLL_INTERVAL (5 PHUT) khien du lieu
    // het "tuoi" sau 5s va lam dong bang toan bo may trang thai AUTO
    // (xem guard "millis()-lastLoraRxTime>5000" trong 2 ham AUTO ben tren).
    // Doi sang SENSOR_POLL_INTERVAL (2s) de du lieu luon tuoi, may trang
    // thai chay lien tuc khong bi dung giua chung.
    if (millis() - lastSensorPollTime >= SENSOR_POLL_INTERVAL) { 
